const App = {
  ...
  _footerHtml() {
    return `<footer class="app-footer"><p>Сайт не использует cookies, которые бы хранили персональные данные пользователей. Сайт не использует метрики. Сайт создан для ведения турнирных таблиц мероприятий Учреждения <strong>СПб ГБУ «ЦФКСиЗ Невского района Санкт-Петербурга»</strong>.</p></footer>`;
  }
};
