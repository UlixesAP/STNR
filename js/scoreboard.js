const ScoreboardApp = {
  ...
  _bindHockey(root, session) {
    ...
    const isPlaying = ['period1','period2','period3'].includes(phase);
    const isOvertime = phase === 'overtime';
    const isShootout = phase === 'shootout';
    const isFinished = phase === 'finished';

    // --- Блок основного счёта ---
    const mainBlock = (isPlaying || isOvertime) ? `
      <div class="score-duel score-duel--football">
        <div class="score-team-zone score-team-zone--football" data-team="A">
          <input type="text" class="score-team-name" data-team-name="A"
            value="${this.esc(session.teamA)}" placeholder="Команда А">
          <div class="score-big" data-score-display="A">${st.scoreA}</div>
          <div class="score-pts-inline">
            <button type="button" class="btn score-pt-btn" data-hk-goal="A:1">+1</button>
            <button type="button" class="btn score-pt-btn btn-ghost" data-hk-goal="A:-1">−1</button>
          </div>
          <div class="score-penalty-box">
            <span class="score-penalty-label">Штрафные минуты: 
              <strong data-hk-pim-display="A">${st.pimA || 0}</strong>
            </span>
            <div class="score-pts-inline">
              <button type="button" class="btn btn-sm btn-warning" data-hk-pim="A:2">+2 мин</button>
              <button type="button" class="btn btn-sm btn-danger"  data-hk-pim="A:5">+5 мин</button>
              <button type="button" class="btn btn-sm btn-ghost"   data-hk-pim="A:-2">−2</button>
            </div>
          </div>
        </div>
        <div class="score-team-zone score-team-zone--football" data-team="B">
          <input type="text" class="score-team-name" data-team-name="B"
            value="${this.esc(session.teamB)}" placeholder="Команда Б">
          <div class="score-big" data-score-display="B">${st.scoreB}</div>
          <div class="score-pts-inline">
            <button type="button" class="btn score-pt-btn" data-hk-goal="B:1">+1</button>
            <button type="button" class="btn score-pt-btn btn-ghost" data-hk-goal="B:-1">−1</button>
          </div>
          <div class="score-penalty-box">
            <span class="score-penalty-label">Штрафные минуты: 
              <strong data-hk-pim-display="B">${st.pimB || 0}</strong>
            </span>
            <div class="score-pts-inline">
              <button type="button" class="btn btn-sm btn-warning" data-hk-pim="B:2">+2 мин</button>
              <button type="button" class="btn btn-sm btn-danger"  data-hk-pim="B:5">+5 мин</button>
              <button type="button" class="btn btn-sm btn-ghost"   data-hk-pim="B:-2">−2</button>
            </div>
          </div>
        </div>
      </div>
      <div class="score-actions-row">
        ${phase === 'period1' ? `<button type="button" class="btn btn-success" data-hk-end-period="1">Завершить 1-й период</button>` : ''}
        ${phase === 'period2' ? `<button type="button" class="btn btn-success" data-hk-end-period="2">Завершить 2-й период</button>` : ''}
        ${phase === 'period3' ? `<button type="button" class="btn btn-success" data-hk-end-period="3">Завершить 3-й период</button>` : ''}
        ${isOvertime ? `<button type="button" class="btn btn-success" data-hk-end-overtime>Завершить овертайм</button>` : ''}
      </div>` : '';

    // --- История периодов ---
    const historyRows = st.periods.map(p => `
      <div class="score-history-row">
        <span>${p.period}-й период</span>
        <strong>${p.scoreA} : ${p.scoreB}</strong>
        <span class="muted-desc">ШМ: ${p.pimA||0} / ${p.pimB||0}</span>
      </div>`).join('');

    const historyBlock = st.periods.length ? `
      <div class="score-history">
        <h3 class="section-title">Результаты периодов</h3>
        ${historyRows}
        <div class="score-history-row score-history-row--total">
          <span>Итого</span>
          <strong>${st.scoreA} : ${st.scoreB}</strong>
        </div>
      </div>` : '';

    // --- Ничья после 3-го периода ---
    const drawPrompt = (phase === 'draw_prompt') ? `
      <div class="score-draw-prompt glass">
        <p>Ничья после основного времени (${st.scoreA}:${st.scoreB}). Что дальше?</p>
        <button type="button" class="btn btn-success" data-hk-goto-overtime>Овертайм</button>
        <button type="button" class="btn btn-warning" data-hk-goto-shootout>Серия буллитов</button>
        <button type="button" class="btn btn-ghost"   data-hk-finish-draw>Завершить (ничья)</button>
      </div>` : '';

    // --- Буллиты ---
    const shootoutBlock = (isShootout || st.penaltyShootouts?.shotsA?.length) ? (() => {
      const pa = st.penaltyShootouts?.shotsA || [];
      const pb = st.penaltyShootouts?.shotsB || [];
      const sA = pa.filter(Boolean).length;
      const sB = pb.filter(Boolean).length;
      return `
        <section class="glass scoreboard-card scoreboard-card--sub">
          <h3 class="section-title">Серия буллитов</h3>
          <p class="score-penalty-total">
            ${this.esc(session.teamA)}: <strong>${sA}</strong> · 
            ${this.esc(session.teamB)}: <strong>${sB}</strong>
          </p>
          <div class="score-penalty-grid">
            <div>
              <p class="score-team-label">${this.esc(session.teamA)}</p>
              <button type="button" class="btn btn-sm btn-success" data-hk-shot="A:1">Забил</button>
              <button type="button" class="btn btn-sm btn-danger"  data-hk-shot="A:0">Мимо</button>
            </div>
            <div>
              <p class="score-team-label">${this.esc(session.teamB)}</p>
              <button type="button" class="btn btn-sm btn-success" data-hk-shot="B:1">Забил</button>
              <button type="button" class="btn btn-sm btn-danger"  data-hk-shot="B:0">Мимо</button>
            </div>
          </div>
          <p class="muted-desc">Удары: ${pa.length} / ${pb.length}</p>
          ${pa.length >= 3 && pb.length >= 3 && !st.penaltyShootoutsFinished
            ? `<button type="button" class="btn btn-warning" data-hk-end-shootout>Завершить серию буллитов</button>`
            : ''}
        </section>`;
    })() : '';

    // --- Итог матча ---
    const summaryBlock = isFinished ? `
      <section class="glass scoreboard-card">
        <h3 class="section-title">Итог матча</h3>
        <div class="score-summary">
          <p>Счёт: <strong>${st.scoreA} : ${st.scoreB}</strong></p>
          <p>Штрафные минуты: <strong>${st.pimA||0} / ${st.pimB||0}</strong></p>
          ${st.overtimePeriods?.length 
            ? `<p>Овертайм: <strong>${st.overtime?.scoreA||0} : ${st.overtime?.scoreB||0}</strong></p>` 
            : ''}
          ${st.penaltyShootouts?.shotsA?.length
            ? `<p>Буллиты: <strong>
                ${st.penaltyShootouts.shotsA.filter(Boolean).length} : 
                ${st.penaltyShootouts.shotsB.filter(Boolean).length}
               </strong></p>`
            : ''}
          ${st.winner 
            ? `<p class="score-winner">Победитель: <strong>${this.esc(st.winner)}</strong></p>` 
            : '<p>Результат: ничья</p>'}
        </div>
      </section>` : '';
  ...
  _bindHockey(root, session) {
    ...
    // --- Голы ---
    root.querySelectorAll('[data-hk-goal]').forEach(btn => {
      btn.addEventListener('click', () => {
        syncNames();
        const [side, delta] = btn.dataset.hkGoal.split(':');
        const d = parseInt(delta, 10);
        if (side === 'A') st.scoreA = Math.max(0, st.scoreA + d);
        else st.scoreB = Math.max(0, st.scoreB + d);
        persist();
    ...
    // --- Штрафные минуты ---
    root.querySelectorAll('[data-hk-pim]').forEach(btn => {
      btn.addEventListener('click', () => {
        syncNames();
        const [side, delta] = btn.dataset.hkPim.split(':');
        const d = parseInt(delta, 10);
        if (!st.pimA) st.pimA = 0;
        if (!st.pimB) st.pimB = 0;
        if (side === 'A') st.pimA = Math.max(0, st.pimA + d);
        else st.pimB = Math.max(0, st.pimB + d);
        persist();
        rerender();
      });
    });
    ...
    // --- Завершение периодов ---
    root.querySelector('[data-hk-end-period]')?.addEventListener('click', (e) => {
      syncNames();
      const periodNum = parseInt(e.target.dataset.hkEndPeriod, 10);
      st.periods.push({
        period: periodNum,
        scoreA: st.scoreA,
        scoreB: st.scoreB,
      ...
      if (periodNum === 1) st.phase = 'period2';
      else if (periodNum === 2) st.phase = 'period3';
      else if (periodNum === 3) {
        if (st.scoreA === st.scoreB) st.phase = 'draw_prompt';
        else {
          st.winner = st.scoreA > st.scoreB ? session.teamA : session.teamB;
          st.phase = 'finished';
        }
      }
      persist();
    ...
    // --- Переходы после ничьей ---
    root.querySelector('[data-hk-goto-overtime]')?.addEventListener('click', () => {
      st.phase = 'overtime';
      st.overtime = { scoreA: 0, scoreB: 0, period: 1 };
      persist();
      rerender();
    });

    root.querySelector('[data-hk-goto-shootout]')?.addEventListener('click', () => {
      st.phase = 'shootout';
      st.penaltyShootouts = { shotsA: [], shotsB: [] };
      persist();
      rerender();
    ...
    root.querySelector('[data-hk-finish-draw]')?.addEventListener('click', () => {
      st.winner = null;
      st.phase = 'finished';
      persist();
      rerender();
    ...
    // --- Завершение овертайма ---
    root.querySelector('[data-hk-end-overtime]')?.addEventListener('click', () => {
      syncNames();
      st.overtimePeriods = st.overtimePeriods || [];
      st.overtimePeriods.push({ scoreA: st.scoreA, scoreB: st.scoreB });
    ...
    // --- Буллиты ---
    root.querySelectorAll('[data-hk-shot]').forEach(btn => {
      btn.addEventListener('click', () => {
        if (st.penaltyShootoutsFinished) return;
        const [side, val] = btn.dataset.hkShot.split(':');
        const scored = val === '1';
        if (!st.penaltyShootouts) st.penaltyShootouts = { shotsA: [], shotsB: [] };
        if (side === 'A') st.penaltyShootouts.shotsA.push(scored);
        else st.penaltyShootouts.shotsB.push(scored);
        persist();
        rerender();
      });
    });

    root.querySelector('[data-hk-end-shootout]')?.addEventListener('click', () => {
      const sA = st.penaltyShootouts.shotsA.filter(Boolean).length;
      const sB = st.penaltyShootouts.shotsB.filter(Boolean).length;
      if (sA === sB) {
        alert('Ничья в серии буллитов — продолжайте.');
        return;
      }
      st.penaltyShootoutsFinished = true;
      st.winner = sA > sB ? session.teamA : session.teamB;
